...
import {emitDeleteDev, emitShareDevToUser, wsListenerMessage, wsRemoveListenerMessage} from "../../../Socket";
import {NeuButton, NeuView} from "../../NeuView";
import CheckBox from "@react-native-community/checkbox";
import Kohana from '../../InputCustom/Kohana';
const ShareDeviceWithEmailCD = () => {
    ...
    const [useDoor, setUseDoor] = useState(true);
    ...
    const submit = () => {
        ...
        let role = 'share_'
        if (useDoor) role += 'cam_door'
        else role += 'cam'
        emitShareDevToUser(email, devId, role, 'share')
    }
        ...
        <Animated.View style={[styles.body, {left}]}>
            ...
            <View style={styles.frame_content}>
                ...
                <View style={{ flexDirection: "row", justifyContent: "flex-start", marginTop: 15, width, paddingHorizontal: 30, alignItems: 'center'}}>
                    <Text style={{ fontWeight: 'bold', fontSize: 16, color: theme.color}}>{LG.allowsDoorControl} </Text>
                    <TouchableOpacity onPress={ () => setUseDoor(!useDoor)}>
                        <CheckBox
                            disabled={true}
                            value={useDoor}
                ...
                <FlatList
                    ...
                    renderItem={ ({ item }) => {
                        ...
                        const textRole = role === 'admin' && "Admin" ||
                                         role === 'use' && LG.use ||
                                         role === 'use_cam' && "camera" ||
                                         role === 'use_cam_door' && "camera & cửa" ||
                                         ['share','share_cam','share_cam_door'].includes(role)  && LG.shading || ""
                        return (
                            <NeuView
...
export default ShareDeviceWithEmailCD
