            ...
            <Animated.View style={[styles.frameSetName, {left}]}>
                ...
                <View style={{flex: 1, paddingHorizontal: 15}}>
                    ...
                    <View style={{ marginTop: 15, paddingLeft: 15}}>
                        <Text style={{ color: theme.color, fontSize: 16}}>{LG.airConditioner}: {dev.status}</Text>
                        <Text style={{ color: theme.color, fontSize: 16}}>{LG.ControllerMode}: {dev.usermode}</Text>
                        <Text style={{ color: theme.color, fontSize: 16}}>{LG.airConditioningMode}: {dev.acmode}</Text>
                        <Text style={{ color: theme.color, fontSize: 16}}>{LG.temperatureControl}: {dev.tctrl}°C </Text>
                    </View>
                    <View style={styles.groupTitleDoor}>
                        <View style={styles.symbolItem}/>
...
const ItemSetup = ({ item, LG, theme, dev }) => {
    ...
    let action = { icon: 'power', txt: ''}
    if (data.data.action === 'pon') action = { icon: 'power', txt: LG.openAirConditioner}
    if (data.data.action === 'poff') action = { icon: 'power', txt: LG.closeAirConditioner}
    return (
        <>
            <NeuView
                ...
                >
                    <View style={{ justifyContent: "space-between"}}>
                        <View style={{ flexDirection: "row", }}>
                            <IconMaterialCommunity name={action.icon} color={theme.color} size={20}/>
                            <Text style={{ fontWeight: '700', marginLeft: 10, color: theme.color }}>
                                { action.txt}
                            </Text>
                        </View>
                        <View style={{ flexDirection: "row", marginTop: 10}}>
                            <IconMaterialCommunity name={'account-voice'} color={theme.color} size={20}/>
                            <Text style={{ fontWeight: '700', marginLeft: 10, color: theme.color }}>
                                { data.txt}
                            </Text>
                        </View>
